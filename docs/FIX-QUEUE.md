...
- [x] **T5 · Inbox Classify and Link to invoice are disabled, and classification works elsewhere** *(high)*
...
- [x] **T6 · A deal's deadline cannot be corrected** *(done inside V4)*
...
- [x] **V0 · Link the screen that already attaches datasheets** *(take first — it is an hour)*
...
- [x] **V1 · Discarding a deal must take its work with it**
...
- [x] **V2 · A delete on the row, not only inside the record**
...
- [x] **V3 · The bin must empty, and the countdown must mean something**
...
- [x] **V4 · A deal can be corrected** *(absorbs T6)*
...
- [x] **U1 · A backup screen, because a promise nobody can see is not a promise**
  ...
  `/mnt/usb-backup`, a Linux path on a Windows machine, so the script ignores it
  and every copy lands in `C:\SUPSERV-ERP\.data\backups` — the disk the
  database is on. It covers a mistake and nothing else. **Run one now** and
  **change the destination** are both wired; **restore is deliberately not**,
  and the screen says why. Reachable from `/settings` under Control with its
  live state as the chip, and from the backup card on `/settings/storage`.
  Two things nobody could see before are now stated: `.env` names an off-site
  SharePoint folder and nothing anywhere uploads to it, and changing `BACKUP_AT`
  does not move the scheduled task until `install-services.ps1` is run again.
...
- [x] **U2 · Everything that expires, in one place**
  `/compliance/expiring` — one table ordered by date, four sources, every row
  naming what it breaks and linking to it. `companyCredential.expiresOn` against
  each OPEN tender's own closing time (the `expiresBeforeDeposit` state, company
  wide rather than per folder), `projectCaution.expiresOn` while unreleased,
  `personCertification.expiresOn` against the live crew, and an issued offer's
  `issued_on + valid_days`. Computed, never stored, and the thirty-day window is
  imported from `dossier.ts` so screen 08 and this cannot drift. It lives under
  Compliance, which is thin and already in the rail, with the primary button and
  a strip of its worst numbers up there that is silent when there is nothing to
  say. What carries a duration and no date is named in a panel rather than
  omitted. **It exposes one gap**: a company paper can be filed only from a
  tender's folder, so a credential no open tender asks for has nowhere to be
  renewed — under Known gaps.
...
- [!] **U3 · Settings a person can read**
  **READ THIS BEFORE TOUCHING IT — it was implemented, and then the rule
  changed under it.** The scheduled run of 9 September took U3 at 17:55, built
  it, got `pnpm check` green (145 files, 1876 tests) and pushed **`b4724b2`** at
  18:15. Abdou's design-first rule landed at 18:16 as `089d1a3`, one commit
  later, and marked this task `[!]`. So the work exists, on the branch, green —
  it is not half-written and there is nothing dangling.
  **It has NOT been reverted, and that is a decision rather than a preference.**
  Reverting a green pushed commit and re-deciding it are both Abdou's calls, and
  the loop forbids guessing at either. The question is under **Needs Abdou**.
  **Nothing was built after that commit.** The run stopped at the rule, and
  every remaining task in this queue now has a visible surface, so the whole
  queue is waiting on frames.
  What `b4724b2` actually changed, so a Figma frame can be drawn for or against
  it: nine rows that were a name and a link now carry a chip — your role, your
  interface language, imports that landed, whether the working disk takes a
  write, unconfirmed compliance rules, records in the bin, when "clear my test
  data" last ran, how many assistant tools your role holds, and the website
  form's "Not built". The group **Control** became **If something goes wrong**
  (backup, bin, clear, audit); compliance moved to the company group and the
  assistant to "what this system does, and does not"; every group gained a
  sentence under its heading, both languages.
  **THE FRAME EXISTS AND IS WAITING FOR YOU.** Drawn 9 September after the rule
  landed, on the `v4 - Complete` page, from that file's own tokens and its
  `Sidebar` and `Topbar` components. Nothing existing was touched — the page has
  two more frames than it had, not two different ones:
  - **`29b - Settings hub (U3 · for approval)`** — the whole screen, five groups,
    twenty-three rows, every chip.
    `https://www.figma.com/design/p0zcsZTobPL8zZAeYhyYBt/?node-id=166-14459`
  - **`29b - review note (U3)`** beside it — what changed and why, so the frame
    can be judged without reading a diff.
    `https://www.figma.com/design/p0zcsZTobPL8zZAeYhyYBt/?node-id=169-14642`
  The numbers on the chips are invented. The shapes, the tones and the words are
  the ones the screen actually uses.
  Note the frame is the HUB, not the tabbed editor screen 29 draws. That
  divergence predates this task: `/settings` refuses to be a second editor of
  rows day one already owns, and its own docstring says why. If you want the
  tabs back, that is a bigger decision than U3 and it needs its own line here.
...
**A company paper can be filed from a tender's folder and from nowhere else.**
`saveCredentialAction` lives in `tenders/[id]/actions.ts` and has no other
caller, so `company_credential` — the CNAS attestation, the extrait de rôle, the
certificat de qualification, held ONCE and read by every open folder — can only
be renewed by opening a tender that happens to ask for it. Screen 27b says so on
a greyed button when no open tender does, which is honest and is not a fix. What
it costs: a `/settings/credentials` hub listing all nine keys with their expiry
and their scan, reusing `saveCredential` unchanged and the same
`settings.company` permission. Half a day, and it is the natural first row of
whatever U3 does to the settings hub.

**Nothing off-site, and `.env` says otherwise.** `BACKUP_SHAREPOINT_FOLDER` is
set to `/Backups/ERP` and there is no code anywhere in this repository that
uploads a backup to SharePoint, or to anything else. Screen 66b now states that
plainly, which turns an invisible false promise into a visible gap. The fix is
either the upload — the same Graph permission conversation the storage screen
has been waiting on since August — or an external drive, which is a plug and a
path typed into the destination box on that screen.

**`BACKUP_AT` is read once, by the installer.** `install-services.ps1` reads it
to build the `SUPSERV backup` trigger; `backup.ps1` never looks at it. So
editing `.env` does not move the nightly run until the installer is run again as
Administrator, and screen 66b says exactly that rather than offering a time box
that would silently do nothing. Making the time editable from the ERP means
re-registering a scheduled task from a web request, which needs Administrator
and is not a thing a page should do — it is a question for Abdou rather than a
task.
...
committed at 18:16.** One minute, and the run had no way to see it coming — it
had held the task since 17:55 and the queue said `[ ]` when it took it. The
commit is green and it is on `fix/usability-2026-09`, one below your own.

**The frame is drawn either way** — `29b - Settings hub (U3 · for approval)` on
`v4 - Complete`, node `166-14459`, with a review note beside it at `169-14642`.
So there is something to say yes or no to before any of the three below.

**Nothing has been reverted, because that is your call and not the loop's.**
Three ways to go and each is one line to whoever runs next:

1. **Keep it.** Mark U3 `[x]` and the design-first rule starts with U4. You
   still get a frame of the new Settings if you want one to react to — the
   screen exists, so the frame can be drawn FROM it rather than before it.
2. **Keep it, but redraw it.** Treat `b4724b2` as the sketch, draw the frame,
   and let the next run reconcile the screen to whatever you approve.
3. **Revert it.** `git revert b4724b2` is clean — six files, no migration, no
   data, and nothing else has been built on top of it.

**The wider question the rule raises, and it is worth answering once:** every
remaining task in this queue has a visible surface. U3, U4 and U5 are `[!]`;
3.1 is `[!]`; 3.6 converts four lists to `DataTable`; 1.9 is blocked on daily
use. So "produce the frames and stop" is now the whole of the work, and a
scheduled run has nothing else it may take. If you would rather the loop kept
moving on things with no pixels in them, say which — the Known gaps below hold
...
|---|---|---|---|
| 2026-09-12 | V2c | `PENDING` | **"I cannot delete an offer."** He was right, and the reason is the shape of V2 rather than a missing capability. `discardDocument` has existed since Wave 0; V2 put a row delete on `/invoices` and on the drafts hanging off an enquiry, and stopped there — because the task was written against the two screens the owner had named. **Four more lists own documents that can be drafts and none of them could remove one**: `/offers`, `/orders`, `/deliveries`, and the offer builder itself. The builder is the worst of the four and the one he actually hit: an offer is opened at `/offers/[id]/build`, worked on there and abandoned there, and the only delete in the system was on `/documents/[id]` — a third screen he had no reason to know existed. So a proforma built for the wrong client could be corrected everywhere except removed. Row delete on all four, drafts only: an issued offer keeps its number for ever, an issued order is the client's own commitment, and a bon de livraison that has gone out is paper somebody signed for — a trash icon beside any of those would exist only to refuse, and the correction for issued paper is an avoir on a different screen. **The general lesson, written down because it has now happened three times in two days** (V0's door with no corridor, V2's delete on two lists out of six, and this): a capability added to the screens a review NAMED is a capability that is missing everywhere else, and the seam between the logic and the screen is still where this system fails. `pnpm check` green: 152 files, 1 959 tests. |
| 2026-09-11 | V2b | `4feeea6` | **Bulk delete, asked for by Abdou — and the rule it breaks had already stopped being true.** `bulk-bar.tsx` has said since screen 79 was drawn: *"Send, issue, cancel and delete are deliberately absent. Do not add them here"*, and the reason it gave was that deletion could not be undone. That was correct when it was written and is not now: V1 made a discard take its work with it AND give all of it back, V2 made every removal say what it takes, V3 made the bin a real bin with a separate Gérant-only one-at-a-time way to finish. Fifty reversible acts are not more dangerous than one, so the premise went and the button arrives. **Send, issue and cancel stay absent for ever** — those leave the building, and nothing that leaves the building happens fifty at a time. The comment in `bulk-bar.tsx` now says all of that rather than contradicting the screen. **What actually carries the safety is not the confirmation.** It is that the preview runs on the SERVER through `bulkDiscardPreview`, using the same guards the discard itself uses, and that a refusal is never silent: the rows that cannot go are NAMED before the button is pressed and named again in the banner afterwards — "FA-2026-0007 — it has been issued; a correction is an avoir". A bulk action that quietly skips eleven of your fifty rows is worse than no bulk action, and that is what the original rule was really protecting against. `bulkDiscard` is deliberately a LOOP, not one statement: each row gets the same guard and its own audit entry it would get alone, because a bulk path with its own shortcut is how an issued invoice eventually reaches the bin. Capped at **50 at a time** — not a technical limit, the loop would do five hundred, but the number above which "I ticked the wrong filter" stops being recoverable by reading the bin. Live on deals, companies, contacts and people; one file of server actions for all four kinds, because the rules are the domain's and four copies would be four places for one to drift. **7 new integration tests** hold the property that matters: the preview writes nothing, the answer before matches the answer after, the invoiced enquiry is untouched, each record gets its own audit entry, and one can be restored alone. `pnpm check` green: 152 files, 1 959 tests. |
| 2026-09-11 | V0b | `72ab0ec` | **Found by looking at the built screens, which is the only way this one could have been found.** V0 gave the catalogue a door and put a technical-file link on every deal line — and the line's cell read *"non rattachée"* on every row of every enquiry. `deal_line.item_id` has existed since phase 4 with a comment saying "set when a person matches this line to the catalogue", and **nothing ever set it**: `grep matchedBy` returns three hits and all three write `null`. So the catalogue could only ever be filled by a seed script, every article read "0 enquiries" for ever, and V0's link could never appear on any line anywhere. The door was built and the corridor to it was not — the same seam this codebase keeps failing at, caught this time because the screens were photographed instead of assumed. `/deals/[id]/items/[lineId]/match` is the corridor: candidates proposed in three passes (our code exactly matching what they wrote, a known alias, then every significant word of the designation) each carrying **the reason it is proposed**, and nothing auto-matched however exact the code looks — "P-01" meaning our article and "P-01" meaning the client's own line numbering are the same string, and half the consultations in Adrar number their lines that way. When it is something never sold before, the line becomes a new article in the same act: the code is OURS and generated (`ART-0001`), never the client's reference, and their reference is kept as an alias so the next enquiry from them matches on the first pass. Three other things the screenshots caught: the Articles list counted `item_coverage` for "enquiries", which a catalogue match does not create, so the column read "—" for every article however many deals used it (counted off `deal_line` now); the owner select offered **"Personne"** for nobody, which in French reads as *a person* and is exactly backwards ("Non attribué"); and the contact select said "Personne d'identifié", which is not French. `pnpm check` green: 151 files, 1 951 tests — and the audit-entity registry caught two new entities before I did, `deal_line` and `item`. |
| 2026-09-10 | T5, V0–V4 | `5ff6eb0` | **Wave V block 1 — delete, edit and the bin, which he chose because nothing else is safe to test until he can undo his own mistakes.** *T5:* the inbox row drew a greyed **Classify** saying "say what this is first" beside rows where classifying plainly worked — `COMMIT_BLOCKER.needsReview` is `null` and the message page has had the reclassify form all along, so the row's only control was a refusal to do the one thing that was not refused. It is a link now, and the two rows that ARE blocked (a payment with no invoice picker, a supplier quote with no sourcing picker) keep the honest refusal AND gain a way into the message beside it. *V0:* `/items/[id]/technical` shipped complete — an upload form wired to `addItemMedia`, five media kinds, provenance on every row — and **nothing in the application linked to it**, so he tested the ERP, found no way to attach a fiche technique, and wrote down that it did not exist. `/items` is the door: the catalogue with what we hold on each item, `hasDatasheet` computed and a GENERIC item never counted as missing one. A link per line on the deal's item table, and a line not yet matched to the catalogue says so rather than showing a dead link. `item_media.deal_id` existed for "a picture the client sent belongs to that enquiry" and **nothing was writing it**; `?deal=` now travels with the link and is checked against a real matched line before it is trusted. *V1:* `discardDeal` wrote ONE row. The foreign keys look like they would save you — `deal_line` is ON DELETE cascade, `sourcing_request` too — and both fire on a HARD delete, which this system never does. So an enquiry left every list while its unissued proforma stayed on `/offers` pointing at a deal nobody could open, and that draft was not in the bin either. Discard now bins the enquiry and its unissued drafts **in one transaction sharing one timestamp**, which is what lets restore put back exactly this act and leave a draft somebody binned separately on Tuesday where it is — no new column, no marker in a reason string. The sourcing list gained the deal-deleted filter the tenders list has always had. The refusal NAMES the paper: "FA-2026-0007 has been issued" instead of "1 issued document". *V2:* the bulk bar still refuses deletion and always will — that rule is about doing fifty at once and had been read as a rule about doing one. `RowDelete` is one record, from the row you are looking at, and the confirmation is not a speed bump: it asks the server what the removal takes with it, when the panel opens rather than once per row, and shows no confirm button at all when the answer is a refusal. On the deals list, on `/invoices` drafts, and on the drafts hanging off an enquiry. *V3:* `BIN_DAYS` counted down and `deletion.ts:543` admitted nothing purged at zero — "gone in 4 days" beside rows that would be there in a year, which teaches a person that the numbers in this system are decoration. `purgeFromBin` is deliberately the narrowest thing that works: one row at a time, `records.purge` (the Gérant alone, stripped from the assistant beside `records.delete`), a typed confirmation of the record's own reference checked on the SERVER, never issued paper, and the audit entry written in the same transaction so it outlives the row. **Structure is not guessed:** 38 foreign keys point at these five tables and 20 are NO ACTION, so rather than hand-maintain a list that would be wrong the first time somebody adds a table, the delete runs in a transaction and Postgres's own refusal is caught and reported with the table it named. Nothing is ever orphaned to make a delete succeed. The countdown counts UP now, a row past thirty days says it is ready, and the bin is a rail row instead of two clicks inside Settings. *V4:* `find src/app -type d -name edit` returned exactly TWO routes in the whole application and neither was a deal — so a deadline read off a PDF and agreed to in a hurry was unfixable for the life of the enquiry, and the deadline is the field that loses the bid. `/deals/[id]/edit` covers subject, client, contact, reference, deadline, submission method, currency, instructions and **owner — `deal.owner_id`'s first writer anywhere**, which is what unblocks "assign to" on the list. The audit entry carries only the fields that actually moved, before and after; saving unchanged writes nothing. **14 new integration tests** in `deal-cascade.test.ts` pin the cascade, the named refusal, the shared timestamp, the separately-binned draft that must NOT come back, and the purge keeping its audit entry. Four of the codebase's own registries caught things before I did — the message-key test, the audit-action names, the Gérant-only permission list and the server-action permission table all failed until the new work was written down in them. `pnpm check` green: 151 files, 1 951 tests. |
...
| 2026-09-09 | U1 | `0246677` | A backup screen, because a promise nobody can see is not a promise. Everything about backups lived in files on the server — the schedule in `BACKUP_AT`, the destination in `BACKUP_LOCAL_PATH`, the last result in `.data\last-backup.json`, the copies in `.data\backups` — and the only screen that mentioned them was a card in the corner of Storage with a chip and four numbers. **The red block at the top is the whole point.** `BACKUP_LOCAL_PATH` is `/mnt/usb-backup`, left over from a plan to host on a Linux VPS; `backup.ps1` refuses a POSIX path on Windows and falls back to `.data\backups`, and it has said so in a `Write-Warning` every night since August into a log nobody opens. The screen says it in a sentence built from the actual state — which of the three refusals applies, what `.env` says, where the next run will really write — then says what it does and does not cover (a wrong delete, yes; a dead disk, a stolen machine or a fire, no) and puts the box that fixes it on the same page. **Run one now** spawns the script detached and writes `.data\backup-run.json`: the run restores its own dump into a scratch database and compares every row, which takes minutes, so nothing waits on it. The marker is answered by the receipt — a receipt newer than the marker means it finished — and after the scheduled task's own two-hour limit with no receipt the screen says a run was started and never wrote one, and names the log, rather than reading "running" for ever. **Change the destination** writes one line of `.env` and nothing else, which is enough because `backup.ps1` reads that file on every run rather than an environment the ERP was started with, so the next backup lands there with nothing restarted. It is stricter than the script on purpose: the script has to tolerate `C:` on a machine nobody has fixed, and a person typing a path into this box is choosing it, so the same drive letter as the ERP is refused with the reason. Existence and writability are checked before the file is touched, the edit is anchored on the one line so every other byte survives, it is written beside `.env` and renamed over it, and **no backup copy is left behind** — `env-secrets.test.ts` explains at length why a `.env.something` holding four secrets is the thing to avoid. **Restoring is not wired, and that is a decision rather than an omission.** `restore.ps1` refuses to overwrite the live database while anything is serving on port 3000 — which is the process a button here would be running inside — so a button could only ever run the harmless half, and a control labelled Restore that cannot restore is worse than none. Both commands are printed, the drill and the real thing, with the sentence that the files are a separate `.zip` and `/settings/storage` is what says whether the two halves match. Nothing is stored (LAW 1): `.env` is the schedule and the destination, the receipt is the last run, the folder listing is how far back the copies go, and `parseCopyName` reads the date out of the file name using the script's own regex rather than a modification time, because copying a folder to a new drive rewrites every mtime. **Two facts nobody could see are now on screen**: `.env` names `BACKUP_SHAREPOINT_FOLDER` and there is no code anywhere that uploads to it, so there is no off-site copy at all; and `BACKUP_AT` is read once by `install-services.ps1` to build the trigger, so changing it moves nothing until that script is run again as Administrator. Twenty-four new unit tests, all pure — the `/mnt/usb-backup` state is proved without a Windows machine, which is the only way it gets proved on every run. |
| 2026-09-09 | U2 | `ac583f7` | Everything that expires, in one place. Expiry decides whether SUPSERV can bid and it was scattered across four tables and four unrelated screens — a CNAS attestation inside one tender's folder, a bank guarantee inside one site, a welder's habilitation on one man's page, an offer's own validity nowhere at all — so the question a person actually asks on a Monday, "what do we have to renew", could not be asked anywhere. **Four sources, all already stored and none invented.** `company_credential.expires_on`, and what it breaks is every OPEN tender whose folder points at it — deposited ones are left out because their folder is what was handed over and a paper expiring afterwards breaks nothing anybody can still fix. `project_caution.expires_on` while unreleased, and **a closed site is deliberately not filtered out**: a retention guarantee nobody went back for outlives the work by a year and the bank keeps charging for it, which `docs/SCREENS.md` names as one of the reasons screen 16 exists. `person_certification.expires_on` against the crew that is on site today. And an issued quotation or proforma's `issued_on + valid_days`, which is the only one of the four that is arithmetic rather than a column. **The verdict is pure and one-directional.** `expiryState` takes days-left and what depends on the row, so the state the whole tender module exists for is proved against a date in September with no database: a paper with ninety days of life left is comfortable by every other reading and useless for a tender deposited in a hundred, and it reads red rather than grey. It is checked ABOVE the thirty-day warning for the same reason `pieceState` checks it above `expiring` — calling it amber would put it under four other amber rows. `EXPIRING_WITHIN_DAYS` is imported from `dossier.ts` rather than copied, so screen 08 and screen 27b cannot come to disagree about one paper. **What is not on the page is a panel, not an omission.** Four things carry a duration and no date — `deal.required_validity_days` and `sourcing_response.validity_days` are days compared against each other on screen 67, the tender's bid bond records requested and received and no expiry at all, and the proforma validity rule is unconfirmed so the system holds no default. That last sentence is read live from `blocking_rule`, so it stops being printed the day an accountant confirms it rather than becoming a stale claim. `price_quote.valid_until` is left out on a different ground and says so: a stale price expires a costing rather than a commitment, and nine papers buried under three hundred prices would be worse than no page. **Where it lives was a judgement.** A route of its own rather than a third panel on screen 27, because 27 answers "what are the rules doing to us today" and this is a work list with rows, dates and links out — one screen doing both is two screens sharing a scrollbar. Under `/compliance` rather than a rail row, because the rail is the thing Wave 3 exists to cut and a person asking "what expires" is asking the Compliance question in a different tense. Screen 27 gets the primary button — it had none — and a strip of the worst numbers that is silent when all three are nought, so nobody has to know 27b exists to be warned by it. **One gap exposed rather than created**: `saveCredentialAction` lives on screen 08 and has no other caller, so a company paper can be filed only from a tender's folder, and a credential no open tender asks for has nowhere in the ERP to be renewed. The row says exactly that on a greyed button instead of linking somewhere that cannot help. Under Known gaps with what it would cost. Eight unit tests, all pure. |
| 2026-09-09 | U3 | `b4724b2` | **Built, green and pushed one minute before the design-first rule arrived — see Needs Abdou before doing anything with it.** Nine of the twenty-three rows on screen 29 carried no chip, so half the settings hub was a name and a link, which is precisely what the file's own docstring says it is not. All nine read something now and every one of them is READ rather than assumed: your role from the table screen 30 writes; the interface language YOU are in, because LAW 4 makes that per-person; how many imports actually landed (`status = 'imported'` only, so a file somebody opened and closed does not count as one); whether the working volume would take a write, red if it refuses and amber while finished documents still land on the same local disk; how many compliance rules are still unconfirmed, which is the one state on that screen a person can change; how many records are recoverable right now; when "clear my test data" last ran, rather than what it would take, because a count of what a sweep WOULD discard depends on a moment nobody has picked; how many of the assistant's tools YOUR role holds, not the registry's total, because it holds exactly the caller's rights; and the website form's plain "Not built". Two new readers, each deliberately cheaper than the screen it borrows from: `countBin()` is five counts rather than `listBin()`'s five selects and a sort, and `storageHeadline()` asks the filesystem the one question a chip can answer instead of walking the whole tree the way screen 66 must. The group **Control** is gone — it held the backup, the bin, the audit log, the compliance rules and the assistant, five answers to five different questions. It is now **If something goes wrong** and holds only that; compliance went to the company group because those rules decide what may be issued, and the assistant to "what this system does, and does not" because nothing on screen 45 can be changed. Every group gained a sentence under its heading in both languages. `Entry.state` is no longer nullable and `tests/unit/settings-hub.test.ts` holds what the type cannot: no row without a state, no group without a sentence, no chip without French. `pnpm check` green at 145 files / 1876 tests. |
| 2026-09-09 | — | `089d1a3` | **Abdou's, not the loop's.** Design first: no screen gets built before he has approved a Figma frame, and U3, U4 and U5 go `[!]` until one exists. The run that had U3 in flight had already pushed it; it stopped there, reverted nothing, and wrote the keep-or-revert question under Needs Abdou rather than deciding it. |
| 2026-09-09 | U3 | Figma, no commit | **The first task built the new way round.** Two frames on `v4 - Complete`, drawn after `089d1a3` landed and drawn from the file's own things rather than beside them: the 39 variables in `SUPSERV Tokens` for every colour, radius and space, and the `Sidebar` and `Topbar` components for the shell. `29b - Settings hub (U3 · for approval)` (`166-14459`) is the whole screen — five groups, twenty-three rows, every chip; `29b - review note (U3)` (`169-14642`) sits beside it and argues for it, so the frame can be judged without reading a diff. Nothing existing was touched: the page has 89 frames where it had 87, and the one accidental edit — the sidebar instance's Dashboard label, caught by reading the canvas back rather than trusting the write — was restored in the same session and never reached the master component. The frame is the HUB rather than the tabbed editor screen 29 draws, and that divergence predates this task: `/settings` refuses to be a second editor of rows day one already owns. It is named on the task so it is a decision rather than a surprise. The numbers on the chips are invented; the shapes, the tones and the words are the ones the screen actually uses. |
